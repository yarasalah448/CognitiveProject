...
def run_game_AI(chromosome, speed, max_score = 20000, no_show = False):
    ...
    draw_game_on_screen(board, score, level, next_piece, falling_piece, FPS)
    # Save the game state
    game_state = [num_used_pieces, removed_lines, score, win]

    return game_state

def draw_game_on_screen(board, score, level, next_piece, falling_piece, speed):
...
def calc_move_info(board, piece, x, r, total_holes_bef, total_blocking_bloks_bef):
    ...
    return [1, max_height, num_removed_lines, new_holes, new_blocking_blocks, piece_sides, floor_sides, wall_sides]
